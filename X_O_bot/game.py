...
def check_is_game_end():
    ...
    if (data[ROW1][COLA] == data[ROW1][COLB]) and (data[ROW1][COLA] == data[ROW1][COLC]) and (
            data[ROW1][COLA] != EMPTY):
        winner = data[ROW1][COLA]
    elif (data[ROW2][COLA] == data[ROW2][COLB]) and (data[ROW2][COLA] == data[ROW2][COLC]) and (
            data[ROW2][COLA] != EMPTY):
        winner = data[ROW2][COLA]
    elif (data[ROW3][COLA] == data[ROW3][COLB]) and (data[ROW3][COLA] == data[ROW3][COLC]) and (
    ...
    # Условия победы по колонкам
            ...
            data[ROW1][COLA] != EMPTY):
        winner = data[ROW1][COLA]
    elif (data[ROW1][COLB] == data[ROW2][COLB]) and (data[ROW1][COLB] == data[ROW3][COLB]) and (
            data[ROW1][COLB] != EMPTY):
        winner = data[ROW1][COLB]
    elif (data[ROW1][COLC] == data[ROW2][COLC]) and (data[ROW1][COLC] == data[ROW3][COLC]) and (
            data[ROW1][COLC] != EMPTY):
        winner = data[ROW1][COLC]
    # Условия победы по диагоналям
    elif (data[ROW1][COLA] == data[ROW2][COLB]) and (data[ROW1][COLA] == data[ROW3][COLC]) and (
            data[ROW1][COLA] != EMPTY):
        winner = data[ROW1][COLA]
